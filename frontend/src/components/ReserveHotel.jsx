...
const ReserveHotel = ({ handleSetOpenModal, hotelID }) => {
  ...
  const { loading, error, data } = useFetch(
    `https://react-hotel-enroll.onrender.com/api/hotels/room/${hotelID}`
  );
    ...
    try {
      await Promise.all(
        selectedRooms.map((roomId) => {
          const res = axios.put(
            `https://react-hotel-enroll.onrender.com/api/rooms/availability/${roomId}`,
            {
              dates: allDates,
            }
          );
  ...
  return (
    <div className="z-10 w-screen h-full fixed top-0 left-0 flex items-center justify-center bg-black bg-opacity-30">
      <div className="h-80 max-w-5xl overflow-y-scroll p-4 bg-slate-50">
        <IoIosCloseCircle
